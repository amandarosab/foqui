...
class PetWindow(QWidget):
    ...
    def __init__(
        ...
        # Cache de frames já escalonados para poupar CPU
    ...
    def _update_frame(self):
        """Atualiza o frame atual da animação utilizando o cache."""
        pixmap = self.pet.get_current_frame()

        if pixmap:
            # Usa o ID nativo do pixmap no cache para evitar buscar nos dicionários internos
            frame_key = pixmap.cacheKey()
    ...
    def _on_animation_changed(self, animation_name: str):
        ...
        # Forçamos a atualização visual imediata, mas sem chamar advance_frame!
        pixmap = self.pet.get_current_frame()
        if pixmap:
            frame_key = pixmap.cacheKey()
            if frame_key not in self._scaled_frames_cache:
    ...
    def set_scale(self, scale: float):
        """Define a escala do pet e atualiza visualmente de forma estática."""
        self.scale = max(0.5, min(2.0, scale))
        self._scaled_frames_cache.clear()
        self._update_size()
        
        # Não chamamos _update_frame() aqui para não quebrar a fila de avanço de frames.
        # Apenas atualizamos a imagem do label.
        pixmap = self.pet.get_current_frame()
        if pixmap:
            scaled_size = int(self.base_size * self.scale)
